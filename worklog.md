...
---
Task ID: cron-round-2026-09-20-6 (QA + features)
Agent: Z.ai Code (recurring webDevReview)
Task: QA sweep, then this round's work: customer purchase-history drawer, dashboard Money-flow (sales vs expenses) trend, command-palette live stock-alerts section, plus mobile overflow fixes and QuickContact compact mode.

Work Log:
- QA sweep (agent-browser): all 13 views render with ZERO page errors; console clean. DEV SERVER DIED mid-round (next process vanished; only agent-browser survived). System auto-restart did not kick in, so added scripts/ensure-dev.sh (idempotent: probes :3000, starts `bun run dev` via setsid if down) and used it before every browser pass. NOTE: background processes started from a single Bash call are reaped between calls — ensure-dev.sh must be re-run per call/session.
- NEW /api/customers/[id]/history: customer profile + stats (orders, totalSpent, avgBasket, credit balance via creditEntry agg, lastVisit), favorite products (top 4 by qty with revenue), payment mix, and latest 30 sales (each with topItems preview, itemCount, paymentMethod, status). CUSTOMERS_VIEW-gated.
- NEW src/components/pos/customer-history-drawer.tsx (Sheet, right): gradient header with initials avatar + compact QuickContact (call/WhatsApp/copy), stat trio tiles (Total spent / Avg basket / Udhaar, color-coded), "Buys most" list with qty progress bars, "How they pay" tinted badges, purchase timeline (invoice, date, method chip, items preview, chevron hover). "Open udhaar book" button appears when balance>0 → closes drawer and opens CreditBookDialog for the same customer (cross-navigation tested). Silent refetch on open (useEffect [open, refetch]) so balance is never stale after payments; refetch is silent so no spinner flicker.
- Customers view: customer name cell is now a button (hover underline + History icon fade-in) opening the drawer; explicit History icon-button added in Actions (before the khata book icon, with title tooltips); count line hints "click a name for purchase history".
- Dashboard Money flow: /api/dashboard now returns expenseSeries bucketed to EXACTLY mirror salesSeries (hour keys `hour-N` for today, UTC day keys otherwise; verified sum(expenseSeries) === rangeExpenses: 27,251). New "Money flow — {range}" card: recharts AreaChart with gradient fills (sales=chart-1, expenses=chart-5), Sales/Expenses legend dots, teal/rose "Net Rs N" chip (tabular-nums) computed rangeSales − rangeExpenses (verified 180,630 − 27,251 = 153,379), gradient hairline top border. Expenses money-card now shows an amber "% of sales" ratio bar (15%).
- Command palette stock section (permission-gated on INVENTORY_VIEW): live "Stock alerts — N [of M]" group while palette is open, fed by BOTH filter=low and filter=out (bug caught: low filter excludes stock=0, so out-of-stock items were initially missing — now merged out-first like the bell); rows show name + "Out of stock" (destructive) or "N unit left"; clicking a row closes palette → navigates to Inventory → adjust dialog pre-filled (same sessionStorage 'pos-adjust-product' + 'pos:adjust-intent' contract as the bell). END-TO-END TESTED: palette → White Sugar row → adjust dialog → set 12 → save → toast, bell badge instantly dropped to 2. Cashier check: palette shows pages but NO stock section.
- MOBILE OVERFLOW BUG FOUND & FIXED (390px): drawer content was 473px inside a 390px sheet. Root cause chain: Radix ScrollArea renders its content wrapper as display:table (min-width:100%), so the stat grid's intrinsic max-content (3×152px tiles) dictated the whole width; every sibling stretched to 473. Fix: replaced ScrollArea with a plain `overflow-y-auto scrollbar-thin` div + min-w-0 on the grid → columns now 111px, content 389px, labels truncate gracefully. Screenshot-verified before/after.
- QuickContact gains `compact` prop (icon-only tel link, title tooltip) used in the drawer header so long names like "Ahmed Store (Wholesale)" fit; phone pill remains on table rows.
- bun run lint: clean. Verified light + dark (money flow gradients + chips adapt), 390px mobile, owner + cashier. Zero page errors. Demo data restored after tests (ledger receivables back to 0; White Sugar restocked to 12 via the dialog UI).

Stage Summary:
- Customers went from a static table to a real 360° profile: click any name → spend stats, favorite items, payment habits, full invoice timeline, and one-tap call/WhatsApp/copy — all inside a drawer that hands off cleanly to the udhaar book.
- The dashboard now shows the net money story (sales vs expenses area chart + net chip + expense ratio bar), and Ctrl+K became a stock-control surface, not just navigation.
- Fixed a real cross-width layout bug (Radix ScrollArea table-layout trap) that also affected desktop.
- Next-round ideas: Urdu localization, PDF Z-report print polish, "sell again" from drawer invoice, dashboard purchases-vs-sales trend merge, per-customer credit limit warnings at POS.
